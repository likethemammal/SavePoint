...
chrome.extension.onRequest.addListener(function(color) {
	chrome.browserAction.setIcon({path: '/src/images/icons/icon16' + color + '.png'});
});

chrome.tabs.onActivated.addListener(function() {
	chrome.tabs.executeScript(null, {file: "src/js/clearData.js"});
	chrome.tabs.executeScript(null, {file: "src/js/checkLoad.js"});
	chrome.tabs.executeScript(null, {file: "src/js/onActivate.js"}, function() {
        chrome.browserAction.setIcon({path: '/src/images/icons/icon16.png'});
    });
	chrome.browserAction.setIcon({path: '/src/images/icons/icon16.png'});
});
...
function handleError(results) {
    if (results === undefined) {
        chrome.browserAction.setIcon({path: '/src/images/icons/icon16r.png'});
    }
}